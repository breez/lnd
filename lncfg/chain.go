...
type Chain struct {
	...
	PrivateBaseFee       lnwire.MilliSatoshi `long:"privatebasefee" description:"The base fee in millisatoshi we will charge for forwarding payments on our private channels"`
	PrivateFeeRate       lnwire.MilliSatoshi `long:"privatefeerate" description:"The fee rate used when forwarding payments on our private channels. The total fee charged is basefee + (amount * feerate / 1000000), where amount is the forwarded amount."`
	PrivateTimeLockDelta uint32              `long:"privatetimelockdelta" description:"The CLTV delta we will subtract from a forwarded HTLC's timelock value on private channels"`
	...
	ChanReserveScript    string              `long:"chanreservescript" description:"A startlark script using chanAmt variable and returning the required remote channel reserve"`
}
